...
class PinBulletWrapper(object):
    def __init__(self, robot_id, pinocchio_robot, joint_names, endeff_names):
        ...
        self.pinocchio_joint_ids = np.array([pinocchio_robot.getJointId(name) for name in joint_names])
        ...
        ## a quick hack for Romeo
        self.bullet_endeff_ids = [6, 13]
        self.pinocchio_endeff_ids = [pinocchio_robot.getFrameId(name) for name in endeff_names]
    ...
    def get_force(self):
        ...
        for i in self.bullet_endeff_ids:
            ...
            if len(cp) > 0:
                for ci in cp:
                    ...
                    active_contacts_frame_ids.append(self.pinocchio_endeff_ids[j])
                    force = np.zeros(6)
        ...
        #                 if (ci[4] == -1):
        #                     print("Normal force at base: ", normal_force)
        #                 else:
        ...
        #     return active_contacts_frame_ids, contact_force
